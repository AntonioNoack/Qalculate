...
*/
static mp_size_t
hgcd_matrix_apply (const struct hgcd_matrix *mat,
		   mp_ptr ap, mp_ptr bp,
		   mp_size_t n)
{
  ...
  if (mn[0][1] == 0)
    {
      ...
      ASSERT (mn[0][0] == 1);
      ASSERT (mat->p[0][0][0] == 1);
      ASSERT (mn[1][1] == 1);
      ASSERT (mat->p[1][1][0] == 1);

      /* Put B <-- B - q A */
      nn = submul (bp, bn, ap, an, mat->p[1][0], mn[1][0]);
    }
  else if (mn[1][0] == 0)
    {
      /* B unchanged, mat = (1, q; 0, 1) */
      ASSERT (mn[0][0] == 1);
      ASSERT (mat->p[0][0][0] == 1);
      ASSERT (mn[1][1] == 1);
      ASSERT (mat->p[1][1][0] == 1);
      ...
      /* Put A  <-- A - q * B */
      nn = submul (ap, an, bp, bn, mat->p[0][1], mn[0][1]);
    }
  else
    {
	 ...
	 B = m10 a + m11 b  ==> a <= B / m10, b <= B / m11. */
      ...
      TMP_ALLOC_LIMBS_3 (tp, modn,
			 sp, modn,
			 scratch, mpn_mulmod_bnm1_itch (modn, modn, mat->n));
      ...
      if (n > modn)
	{
      ...
      mpn_mulmod_bnm1 (tp, modn, ap, n, mat->p[1][1], mn[1][1], scratch);
      mpn_mulmod_bnm1 (sp, modn, bp, n, mat->p[0][1], mn[0][1], scratch);
	...
	MPN_ZERO (sp + n + mn[0][1], modn - n - mn[0][1]);
      ...
      mpn_mulmod_bnm1 (sp, modn, ap, n, mat->p[1][0], mn[1][0], scratch);
      MPN_COPY (ap, tp, nn);
      mpn_mulmod_bnm1 (tp, modn, bp, n, mat->p[0][0], mn[0][0], scratch);
...
mp_size_t
mpn_hgcd_reduce (struct hgcd_matrix *mat,
		 ...
		 mp_ptr tp)
{
  ...
  if (BELOW_THRESHOLD (n, HGCD_REDUCE_THRESHOLD))
    {
      nn = mpn_hgcd (ap + p, bp + p, n - p, mat, tp);
      if (nn > 0)
	/* Needs 2*(p + mat->n) <= 2*(floor(n/2) + ceil(n/2) - 1)
	   = 2 (n - 1) */
	return mpn_hgcd_matrix_adjust (mat, p + nn, ap, bp, p, tp);
    }
  else
    {
      MPN_COPY (tp, ap + p, n - p);
      MPN_COPY (tp + n - p, bp + p, n - p);
      if (mpn_hgcd_appr (tp, tp + n - p, n - p, mat, tp + 2*(n-p)))
	return hgcd_matrix_apply (mat, ap, bp, n);
    }
  return 0;
}

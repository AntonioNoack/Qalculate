...
static int
abs_sub_n_x156 (mp_ptr rp, mp_srcptr ap, mp_srcptr bp, mp_size_t n)
{
  mp_limb_t  x, y;
  while (--n >= 0)
    {
...
static int
abs_sub_add_n_x156 (mp_ptr rm, mp_ptr rp, mp_srcptr rs, mp_size_t n) {
  int result;
  result = abs_sub_n_x156 (rm, rp, rs, n);
...
/*   if (scratch == NULL) scratch = TMP_SALLOC_LIMBS (9 * n + 3); */
  ...
  sign ^= abs_sub_add_n_x156 (v1, v3, pp, n + 1);

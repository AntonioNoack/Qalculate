...
getbits55 (const mp_limb_t *p, mp_bitcnt_t bi, int nbits)
{
  int nbits_in_r;
  mp_limb_t r;
  mp_size_t i;
...
extern int win_size58 (mp_bitcnt_t);
#else
static inline int
win_size58 (mp_bitcnt_t enb)
{
  int k;
...
static void
redcify58 (mp_ptr rp, mp_srcptr up, mp_size_t un, mp_srcptr mp, mp_size_t n, mp_ptr tp)
{
  MPN_ZERO (tp, n);
  MPN_COPY (tp + n, up, un);
	      ...
	      mp_srcptr mp, mp_size_t n, mp_ptr tp)
{
  ...
  this_pp = pp;
  this_pp[n] = 1;
  redcify58 (this_pp, this_pp + n, 1, mp, n, this_pp + n + 1);
  this_pp += n;
  ...
  /* scratch: |   n   |   n   |  bn + n + 1  |  */
  /*          | pp[0] | pp[1] |   redcify58    |  */
  redcify58 (this_pp, bp, bn, mp, n, this_pp + n);
  ...
  expbits = getbits55 (ep, enb, windowsize);
...
mp_size_t mpn_sec_powm_itch (mp_size_t bn, mp_bitcnt_t enb, mp_size_t n){
  int windowsize;

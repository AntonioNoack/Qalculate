...
static mp_size_t
mpn_pow_1_highpart_get (mp_ptr rp, mp_size_t *ignp,
...
char* mpf_get_str (char *dbuf, mp_exp_t *exp, int base, size_t n_digits, mpf_srcptr u){
    ...
    do {
        ...
        DIGITS_IN_BASE_PER_LIMB (rawn, PREC(u) - 1, base);
        max_digits = rawn + 2;
    } while (0)
	...
	if (ue <= n_limbs_needed)
		{
			...
			pn = mpn_pow_1_highpart_get (pp, &ign, (mp_limb_t) base, e, n_limbs_needed + 1, tp);
			if (un > pn)

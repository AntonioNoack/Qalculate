#ifndef ANNO_PROTECT_1137168190
#define ANNO_PROTECT_1137168190
/*
    Qalculate (library)
...
MathStructure *find_abs_x2(MathStructure &mstruct, const MathStructure &x_var) {
	...
	for(size_t i = 0; i < mstruct.size(); i++) {
		MathStructure *m = find_abs_x2(mstruct[i], x_var);
		if(m) return m;
	}
	return NULL;
}
...
int IntegrateFunction::calculate(MathStructure &mstruct, const MathStructure &vargs, const EvaluationOptions &eo) {
	...
	if(vargs.size() < 5 || !vargs[4].number().getBoolean() || definite_integral == 0) {
		...
		MathStructure *mabs = find_abs_x2(mbak, x_var);
...
extern int test_comparisons(const MathStructure &msave, MathStructure &mthis, const MathStructure &x_var, const EvaluationOptions &eo, bool sub = false, int alt = 0);

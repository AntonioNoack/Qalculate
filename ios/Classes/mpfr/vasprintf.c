  ...
  do {                                                                  \
    ...
    if ((flag))                                                         \
      ...
      {                                                                 \
        ...
        fmt_copy[n] = '\0';                                             \
        length = /*gmp_vasprintf*/fprintf (&s, fmt_copy, (ap));                    \
        if (length < 0)                                                 \
          {                                                             \
...
static char*
mpfr_get_str_aux17 (mpfr_exp_t *exp, int base, size_t n, const mpfr_t op,
  ...
  if (spec.prec != 0)
    {
      ...
      MPFR_ASSERTD (nsd != 1);
      str = mpfr_get_str_aux17 (&exp, base, nsd, p, spec);
  ...
  if (dec_info == NULL)
    {
      ...
      MPFR_ASSERTD (np->ip_size == 1); /* thus no integer overflow below */
      nsd = spec.prec < 0 ? 0 : (size_t) spec.prec + np->ip_size;
      str = mpfr_get_str_aux17 (&exp, 10, nsd, p, spec);
      register_string (np->sl, str);
    }
  else
    {
    ...
    /* 1 <= |p| */
    {
      ...
      if (dec_info == NULL)
        { /* this case occurs with mpfr_printf ("%.0RUf", x) with x=9.5 */
          str = mpfr_get_str_aux17 (&exp, 10, spec.prec+exp+1, p, spec);
          register_string (np->sl, str);
        }
      else
        {
  ...
  else
    {
      ...
      if (spec.spec == 'a' || spec.spec == 'A' || spec.spec == 'b')
        ...
        /* %g case */
        {
          ...
          /* Here we cannot call mpfr_get_str_aux17 since we need the full
      ...
      if (spec.spec == '\0')
        ...
        /* output mpfr_prec_t variable */
        {
          ...
          sprintf (format, "%%%s%s%s%s%s%s*.*" MPFR_PREC_FORMAT_TYPE "%c",
                   ...
                   spec.spec);
                   // todo anno fix...
          length = 0;//gmp_asprintf (&s, format, spec.width, spec.prec, prec);

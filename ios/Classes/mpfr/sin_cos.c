 ...
 */
static mpfr_prec_t
reduce15 (mpz_t Q, mpz_srcptr R, mpfr_prec_t prec)
{
  mpfr_prec_t l;
            ...
            mpfr_prec_t prec)
{
  ...
  m += reduce15 (Q0, Q[0], prec);
  m -= reduce15 (T[0], T[0], prec);
  /* multiply by x = p/2^m */
  mpz_mul (S0, T[0], p);
  m -= reduce15 (S0, S0, prec); /* S0 = T[0] * (1 + theta)^2 up to power of 2 */

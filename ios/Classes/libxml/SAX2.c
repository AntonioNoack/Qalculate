...
xmlErrValid5(xmlParserCtxtPtr ctxt, xmlParserErrors error,
 ...
 */
static void LIBXML_ATTR_FORMAT(3,0)
xmlFatalErrMsg5(xmlParserCtxtPtr ctxt, xmlParserErrors error,
...
xmlSAX2GetEntity(void *ctx, const xmlChar *name)
{
	...
	if (ctxt->inSubset == 2) {
		...
		ret = xmlGetDocEntity(ctxt->myDoc, name);
		if (ret != NULL) {
		    xmlFatalErrMsg5(ctxt, XML_ERR_NOT_STANDALONE,
	...
	if (val == 0) {
	    ...
	    xmlFatalErrMsg5(ctxt, XML_ERR_ENTITY_PROCESSING,
	      ...
	      xmlEnumerationPtr tree)
{
	 ...
	 */
	int tmp = ctxt->valid;
	xmlErrValid5(ctxt, XML_DTD_XMLID_TYPE,
	...
	attr = xmlAddAttributeDecl(&ctxt->vctxt, ctxt->myDoc->extSubset, elem,
	   ...
	   (xmlAttributeDefault) def, defaultValue, tree);
        ...
        xmlFatalErrMsg5(ctxt, XML_ERR_INTERNAL_ERROR,
	     ...
	     const xmlChar *publicId, const xmlChar *systemId)
{
	...
	xmlFatalErrMsg5(ctxt, XML_ERR_NOTATION_PROCESSING,
             ...
             const xmlChar *value, const xmlChar *prefix ATTRIBUTE_UNUSED)
{
	...
	if (xmlStrEqual(fullname, BAD_CAST "xml:id")) {
	    ...
	    if (xmlValidateNCName(value, 1) != 0) {
	        xmlErrValid5(ctxt, XML_DTD_XMLID_VALUE,
...
process_external_subset:
	...
	if ((ctxt->myDoc->standalone == 1) &&
		...
		if ((attr->defaultValue != NULL) &&
		    ...
		    if (att == NULL) {
		        xmlErrValid5(ctxt, XML_DTD_STANDALONE_DEFAULTED,
...
xmlSAX2StartElement(void *ctx, const xmlChar *fullname, const xmlChar **atts)
{
	...
	xmlErrValid5(ctxt, XML_ERR_NO_DTD,
		      ...
		      const xmlChar **attributes)
{
    ...
    if (ctxt->validate && (ctxt->myDoc->extSubset == NULL) &&
	 ...
	 ((ctxt->myDoc->intSubset->notations == NULL) &&
	  ...
	  (ctxt->myDoc->intSubset->attributes == NULL) &&
	  (ctxt->myDoc->intSubset->entities == NULL)))) {
	xmlErrValid5(ctxt, XML_DTD_NO_DTD,
	  "Validation failed: no DTD found !", NULL, NULL);
	ctxt->validate = 0;
    }
